window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const CSS = [
			...
			"body.dsh-zen-active [data-maid-character],body.dsh-zen-active [data-skin-chrome],body.dsh-zen-active [data-skin-trim-layer],body.dsh-zen-active [data-skin-corner],body.dsh-zen-active [data-skin-ornament],body.dsh-zen-active [data-skin-decoration],body.dsh-zen-active [data-skin-*]{display:none !important}",
			// zen HUD
			...
			"#dsh-zen-diag{position:fixed;right:12px;bottom:8px;z-index:2147483005;font-size:10px;color:rgba(0,0,0,.35);pointer-events:none;font-family:ui-monospace,Menlo,monospace}",
			"body[data-ds-dark-theme] #dsh-zen-diag{color:rgba(255,255,255,.3)}",
			// breathing workspace
		...
		function apply(ctx) {
			...
			const refreshHud = function () {
				...
				if (diagEl) {
					...
					diagEl.textContent = "dsh-zen v4  shell=" + (document.querySelector("[" + SHELL_MARK + "]") ? "Y" : "N") + "  chrome=" + hidden.length + "  seat=" + (composerSeat() ? "Y" : "N") + "  ta=" + (ta ? "Y" : "N") + "  skin=" + skinCount;
				}
			};
			...
			const setZen = function (on) {
				...
				if (on) {
					...
					const skinEls = document.querySelectorAll("[data-maid-character],[data-skin-chrome],[data-skin-trim-layer],[data-skin-corner],[data-skin-ornament],[data-skin-decoration],[data-skin-*]");
					for (let i = 0; i < skinEls.length; i++) {
						if (hidden.indexOf(skinEls[i]) === -1) hidden.push(skinEls[i]);
					}
					// background-image sweep: large decorative backdrops (skin art) —
					...
					for (let i = 0; i < allEls.length; i++) {
						...
						if (rr.width < vw * 0.4 || rr.height < vh * 0.4) continue;
					...
					// pseudo-element backdrops (::before/::after full-screen art)
