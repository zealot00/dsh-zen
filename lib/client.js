window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function composerTextarea() {
			...
			// prefer the VISIBLE one (hidden helper textareas fool naive selection)
		...
		function isInputZone(el) {
			if (!el) return false;
			if (el.closest("[data-composer-seat]")) return true;
			return !!el.querySelector("textarea,[contenteditable],[role=textbox],input[type=text]");
		}
		...
		function apply(ctx) {
			...
			const refreshHud = function () {
				...
				if (diagEl) {
					...
					const msg = "dsh-zen v4.9  shell=" + (document.querySelector("[" + SHELL_MARK + "]") ? "Y" : "N") + "  chrome=" + hidden.length + "  seat=" + (composerSeat() ? "Y" : "N") + "  ta=" + (ta ? "Y" : "N") + "  skin=" + skinCount + "  skinOff=" + (state.skinScopeActive ? "Y" : "N");
			...
			const setZen = function (on) {
				...
				if (on) {
					findShell();
					// switch to the default skin: remove the skin's scope attributes —
					// all its CSS lives under body[data-dsh-maid-atelier] etc, so this
					// deactivates the whole skin in one shot (sweeps below are the net)
					state.skinScopeSaved = {};
					const scopes = ["data-dsh-maid-atelier", "data-maid-composer-motion", "data-maid-sidebar-compact", "data-maid-sidebar-size"];
					...
					// attribute-prefix walk: any data-skin-* decorated element — but NEVER
					// input zones, chat log, or our own HUD (data-skin-owner sits on the
					// composer card, so guards are mandatory)
					const skinWalk = document.querySelectorAll("body *");
					for (let i = 0; i < skinWalk.length; i++) {
						const el = skinWalk[i];
						if (hidden.indexOf(el) !== -1) continue;
						if (el.closest("#" + ZEN_ID) || el.closest("[data-dsh-zen-hud]")) continue;
						if (isInputZone(el)) continue;
						if (el.closest("[data-conversation-scroll]")) continue;
						const attrs = el.attributes;
						let hit = false;
						for (let j = 0; j < attrs.length; j++) {
							if (attrs[j].name.indexOf("data-skin-") === 0) { hit = true; break; }
						}
						if (hit) hidden.push(el);
					}
					...
					for (let i = 0; i < allEls.length; i++) {
						...
						if (rr.width < innerW * 0.2 || rr.height < innerH * 0.2) continue;
						...
						// inline SVG/canvas artwork at significant size = skin art
						if (el.tagName === "SVG" || el.tagName === "CANVAS") {
							if ((el.textContent || "").trim().length < 60) { hidden.push(el); continue; }
						}
					...
					// edge-decor sweep: fixed/absolute image layers hugging viewport edges
					// (the whale's feet live in such a small standalone layer)
					const edgeAll = document.querySelectorAll("body *");
					for (let i = 0; i < edgeAll.length; i++) {
						...
						const nearEdge = er.left <= 8 || (innerW - (er.left + er.width)) <= 8 || er.top <= 8 || (innerH - (er.top + er.height)) <= 8;
						if (!nearEdge) continue;
						const hasImg = ecs.backgroundImage !== "none" || el.tagName === "IMG" || el.tagName === "CANVAS" || el.tagName === "SVG" || el.querySelector("img,canvas,svg,video");
						if (!hasImg) continue;
						const eText = (el.textContent || "").trim().length;
						if (eText > 30) continue;
						// style application is deferred to the shared hidden.forEach below
						hidden.push(el);
					}
					// detective mode: EVERY image-ish element (any position, any size) —
					// the whale's feet are not a fixed/absolute layer, so search wider
					const suspects = [];
					for (let i = 0; i < edgeAll.length; i++) {
						const el = edgeAll[i];
						const sr = el.getBoundingClientRect();
						if (sr.width < 8 || sr.height < 8) continue;
						const scs = window.getComputedStyle(el);
						const sImg = scs.backgroundImage !== "none" || el.tagName === "IMG" || el.tagName === "CANVAS" || el.tagName === "SVG" || el.querySelector("img,canvas,svg,video");
						if (!sImg) continue;
						const sText = (el.textContent || "").trim().length;
						if (sText > 120) continue;
						const hiddenNow = hidden.indexOf(el) !== -1;
						suspects.push({
							tag: el.tagName,
							...
							at: Math.round(sr.left) + "," + Math.round(sr.top),
							bg: (scs.backgroundImage || "").slice(0, 40),
							hidden: hiddenNow ? "Y" : "n"
						});
					}
					console.table(suspects);
					// detective: elements carrying data-skin-*/data-maid-* attributes (name=value)
					const skinRows = [];
					for (let i = 0; i < edgeAll.length; i++) {
						const el = edgeAll[i];
						const attrs = [];
						for (let j = 0; j < el.attributes.length; j++) {
							...
							if (a.name.indexOf("data-skin-") === 0 || a.name.indexOf("data-maid-") === 0) {
								attrs.push(a.name + "=" + String(a.value).slice(0, 26));
							}
						}
						...
						skinRows.push({
							...
							size: Math.round(sr.width) + "x" + Math.round(sr.height),
							at: Math.round(sr.left) + "," + Math.round(sr.top),
							hidden: hidden.indexOf(el) !== -1 ? "Y" : "n"
						});
					}
					console.table(skinRows);
					...
					if (ta) {
						...
						// visible zen restyle (inline beats any skin rule)
						ta.dataset.dshZenBr = ta.style.borderRadius;
						ta.dataset.dshZenBorder = ta.style.border;
						ta.dataset.dshZenPad = ta.style.padding;
						ta.dataset.dshZenBg2 = ta.style.background;
						ta.style.borderRadius = "16px";
						ta.style.border = "1px solid rgba(0,0,0,.10)";
						ta.style.padding = "12px 16px";
						ta.style.background = "rgba(255,255,255,.72)";
					}
					...
					clean();
					// restore the skin scope attributes (back to the skin)
