window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function composerTextarea() {
			...
			if (seat) {
				const ts = seat.querySelectorAll("textarea,[contenteditable=true],[role=textbox],input[type=text]");
				for (let i = 0; i < ts.length; i++) add(ts[i]);
			} else {
				const ts = document.querySelectorAll("textarea,[contenteditable=true],[role=textbox],input[type=text]");
				for (let i = 0; i < ts.length; i++) add(ts[i]);
			}
