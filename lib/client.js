window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const CSS = [
			...
			"#dsh-zen-delivery .dz-out{margin-top:10px;padding:10px 12px;background:#f7f8fa;border-radius:10px;font-size:12.5px;color:#555;line-height:1.7;max-height:150px;overflow:hidden;position:relative}",
			"body[data-ds-dark-theme] #dsh-zen-delivery .dz-out{background:#2a2c31;color:#c2c4ca}",
			"#dsh-zen-delivery .dz-out::after{content:'';position:absolute;left:0;right:0;bottom:0;height:34px;background:linear-gradient(transparent,#f7f8fa)}",
			"body[data-ds-dark-theme] #dsh-zen-delivery .dz-out::after{background:linear-gradient(transparent,#2a2c31)}",
			"#dsh-zen-delivery .dz-open{display:inline-block;margin-top:8px;font-size:12px;color:#1a6cbb;cursor:pointer;border:none;background:none;padding:0}",
			"#dsh-zen-delivery .dz-open:hover{text-decoration:underline}",
		...
		function apply(ctx) {
			...
			const showDelivery = function (title, statsHtml, toolsText, noteText, outPreview, outChars) {
				if (!deliveryEl) return;
				const staleOpen = deliveryEl.querySelector(".dz-open");
				...
				const outBox = deliveryEl.querySelector(".dz-out");
				if (outPreview) {
					outBox.textContent = outPreview;
					const openBtn = document.createElement("button");
					openBtn.className = "dz-open";
					openBtn.textContent = "查看完整成果 →";
					openBtn.addEventListener("click", function () {
						const sc = scrollArea();
						if (sc) sc.scrollTop = sc.scrollHeight;
						hideDelivery();
					});
					outBox.parentNode.insertBefore(openBtn, outBox.nextSibling);
				} else {
					outBox.textContent = "";
				}
				...
				deliveryTimer = setTimeout(hideDelivery, 11000);
			};
			...
			const state = {
				...
				turn: { tools: 0, toolNames: [], lastToolAt: 0, startedAt: 0, endedAt: 0, outChars: 0, outPreview: "", lastWasTool: false },
				deliveryShown: false
			};
			...
			const refreshHud = function () {
				...
				if (!running && state.lastDelta) {
					if (!state.turn.lastWasTool) pushHistory(state.lastDelta);
					state.lastDelta = "";
				}
				...
				if (!running && state.turn.endedAt && !state.deliveryShown) {
					...
					const stats = "<span>" + state.turn.tools + " 次工具调用</span><span>" + dur + " 分钟</span>" + (state.turn.outChars ? "<span>" + state.turn.outChars + " 字产出</span>" : "") + (pending ? "<span>▲ 需要你判断</span>" : "<span>✓ 全部完成</span>");
					const tools = state.turn.toolNames.slice(0, 6).join(" · ");
					showDelivery(state.title || "当前任务", stats, tools ? "工具：" + tools : "", pending ? "Agent 留下了需要你判断的问题，点击上方黄色提示查看。" : "", state.turn.outPreview, state.turn.outChars);
				}
					...
					for (let i = 0; i < mutations.length; i++) {
						...
						if (tmatch) {
							...
							if (tool.length > 1 && tool.length < 40) {
								...
								state.turn.lastWasTool = true;
								continue;
							}
						}
						// plain output: accumulate the artifact preview
						state.lastDelta = clean.slice(0, 160);
						if (state.running && clean.length > 6) {
							state.turn.outChars += clean.length;
							if (state.turn.outChars > 99999) state.turn.outChars = 99999;
							state.turn.outPreview = clean.slice(0, 240);
						}
						state.turn.lastWasTool = false;
					}
				});
